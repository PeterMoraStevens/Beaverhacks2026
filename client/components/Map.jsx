...
function MiniRouteMap({ coords, analysisData }) {
  ...
  useEffect(() => {
    ...
    const maxC = analysisData.maxCount || 1;
    ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.lineWidth = 3.5;
    for (let i = 0; i < coords.length - 1; i++) {
      ctx.strokeStyle = intensityToColor((counts[i] || 0) / maxC);
      ctx.beginPath();
    ...
    ctx.beginPath(); ctx.arc(toX(coords[0][0]), toY(coords[0][1]), 5, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#E63946";
    ctx.beginPath(); ctx.arc(toX(coords[coords.length - 1][0]), toY(coords[coords.length - 1][1]), 5, 0, Math.PI * 2); ctx.fill();
    ctx.shadowBlur = 0;
  }, [coords, analysisData]);
  return (
    <canvas ref={canvasRef} style={{ width: "100%", height: "120px", borderRadius: 12, background: "rgba(255,255,255,0.07)", display: "block" }} />
  );
}
...
export default function MapView() {
  ...
  const [tripHistory, setTripHistory] = useState(() => {
    ...
    try { return JSON.parse(localStorage.getItem("trip_history") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      // Route line — lineMetrics enables gradient coloring
  ...
  const applyAvoidanceRadius = (routes, allCams, radius) => {
    ...
    // Count near primary route for header — heatmap stays untouched
  ...
  function addPoint(coord) {
    ...
    if (prependModeRef.current) {
      prependModeRef.current = false; setPrependModeState(false);
  ...
  const clearRoute = () => {
    ...
    pointsRef.current = [];
    prependModeRef.current = false; setPrependModeState(false);
    setPoints([]); setCameras([]); setActiveEndpoint(null);
    setAnalysisData(null); setRouteCoords(null); setShowResults(false); setAnimating(false);
    clearCamerasFromStorage();
    updateRoute([]); updateSafeRouteLayer([]);
    setRouteStats(null); setStatsVisible(true);
    allRoutesRef.current = []; allCamerasRef.current = [];
  ...
  const analyzeRoute = async () => {
    ...
    setAnalyzing(true);
    setRouteStats(null); setAnalysisData(null); setShowResults(false);
    ...
    try {
      ...
      saveTripToHistory({
        id: tripNow, timestamp: new Date(tripNow).toISOString(),
        mode, profile, waypoints: [...waypoints],
  ...
  const switchMode = (newMode) => {
    ...
    cameraMarkersRef.current.forEach(m => m.remove()); cameraMarkersRef.current = [];
    pointsRef.current = [];
    prependModeRef.current = false; setPrependModeState(false);
    setPoints([]); setCameras([]); setActiveEndpoint(null);
    setAnalysisData(null); setRouteCoords(null); setShowResults(false); setAnimating(false);
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", gap: 3, background: "#f0f0f0", borderRadius: 8, padding: 3, flexShrink: 0 }}>
          {[["draw", "✏️ Free Draw"], ["route", "🗺 A→B Route"]].map(([m, label]) => (
            <button key={m} onClick={e => { e.stopPropagation(); switchMode(m); }} style={{
              background: mode === m ? "white" : "transparent", border: "none",
        ...
        {/* History button */}
        <button onClick={e => { e.stopPropagation(); setHistoryOpen(o => !o); }} style={{
          background: historyOpen ? "#1976D2" : "white", border: "1px solid #e0e0e0",
          padding: "7px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600,
          cursor: "pointer", color: historyOpen ? "white" : "#555",
        ...
        <span style={{
          ...
          {cameras.length > 0
            ...
            : isLoading
              ? animating ? "Analyzing your route…" : "Fetching cameras…"
              : mode === "route"
                ? points.length === 0 ? "Click map to place start (A)"
                  : points.length === 1 ? "Click map to place end (B)"
                  : "A→B set — click Analyze Route"
                : points.length === 0 ? "Click map to add points"
                  : activeEndpointId
                    ? prependMode ? "⬆️ Adding before first point — click map"
          ...
          <ResponsiveContainer width="100%" height={70}>
            <AreaChart data={analysisData.graphData} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={v => [`${v} cameras`, "Exposure"]}
                labelFormatter={l => `${l} mi`}
              />
              <Area type="monotone" dataKey="cameras" stroke="#E63946" strokeWidth={2}
                fill="url(#exposureGradient)" isAnimationActive animationDuration={1000} />
            </AreaChart>
          </ResponsiveContainer>
      ...
      {routeStats && !statsVisible && (
        <div onClick={e => e.stopPropagation()} style={{ position: "absolute", bottom: 90, left: "50%", transform: "translateX(-50%)", zIndex: 10 }}>
          <button onClick={() => setStatsVisible(true)} style={{
            background: "white", border: "1px solid #e0e0e0", padding: "8px 18px",
            borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
            boxShadow: "0 2px 12px rgba(0,0,0,0.12)", color: "#555",
            display: "flex", alignItems: "center", gap: 6,
          }}>
            📊 Show Results — {routeStats.original.cameras} camera{routeStats.original.cameras !== 1 ? "s" : ""} on route
          </button>
        </div>
      )}
      ...
      {routeStats && statsVisible && (
        <div onClick={e => e.stopPropagation()} style={{
          ...
          minWidth: 360, display: "flex", flexDirection: "column", gap: 14,
        }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: -4 }}>
            <span style={{ fontWeight: 700, fontSize: 13, color: "#333", letterSpacing: "-0.2px" }}>Route Analysis</span>
            <button onClick={() => setStatsVisible(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#999", fontSize: 16, padding: "0 2px", lineHeight: 1 }}>✕</button>
          </div>

          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 10, background: "#fff5f5", border: "1px solid #fcc" }}>
              <div style={{ fontSize: 11, color: "#999", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.5px" }}>Your Route</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#E63946" }}>{formatTime(routeStats.original.time)}</div>
              <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>{formatDistance(routeStats.original.distance)}</div>
              <div style={{ fontSize: 12, color: "#c47c00", marginTop: 4, fontWeight: 600 }}>📷 {routeStats.original.cameras} camera{routeStats.original.cameras !== 1 ? "s" : ""}</div>
            </div>
            <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 10, background: routeStats.safe ? "#f0f7ff" : "#f9f9f9", border: `1px solid ${routeStats.safe ? "#90caf9" : "#eee"}` }}>
              <div style={{ fontSize: 11, color: "#999", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.5px" }}>Safe Route</div>
              {routeStats.safe ? (
                <>
                  <div style={{ fontSize: 22, fontWeight: 800, color: "#2196F3" }}>{formatTime(routeStats.safe.time)}</div>
                  <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>{formatDistance(routeStats.safe.distance)}</div>
                  <div style={{ fontSize: 12, color: "#388e3c", marginTop: 4, fontWeight: 600 }}>📷 {routeStats.safe.cameras} camera{routeStats.safe.cameras !== 1 ? "s" : ""}</div>
                </>
              ) : (
                <div style={{ fontSize: 13, color: "#999", marginTop: 12 }}>No safer<br />alternative found</div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input type="range" min={10} max={200} step={5} value={safeRadius}
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", gap: 3, background: "white", borderRadius: 10, padding: 3, border: "1px solid #e0e0e0", boxShadow: "0 2px 12px rgba(0,0,0,0.12)" }}>
          {[["walking", "🚶 Walk"], ["driving", "🚗 Drive"]].map(([p, label]) => (
            <button key={p} onClick={e => { e.stopPropagation(); setProfile(p); }} style={{
              background: profile === p ? "#2196F3" : "transparent", border: "none",
              padding: "9px 14px", borderRadius: 7, fontSize: 12,
              fontWeight: profile === p ? 700 : 500, cursor: "pointer",
              color: profile === p ? "white" : "#666", transition: "all 0.15s",
            }}>{label}</button>
          ))}
        </div>
        ...
        <button onClick={e => { e.stopPropagation(); analyzeRoute(); }}
          ...
          style={{
            ...
            boxShadow: canAnalyze && !isLoading ? "0 2px 12px rgba(230,57,70,0.4)" : "none",
            display: "flex", alignItems: "center", gap: 6,
          }}>
      ...
      {historyOpen && (
        <div onClick={e => e.stopPropagation()} style={{
          position: "absolute", top: 61, right: 0, bottom: 0, width: 320,
          ...
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {tripHistory.length > 0 && (
                <button onClick={() => { localStorage.removeItem("trip_history"); setTripHistory([]); }}
