...
function scoreRouteByCameras(routeCoords, cameras, radiusMeters) {
  return cameras.filter((c) => isCameraNearPath(c.lat, c.lon, routeCoords, radiusMeters)).length;
}
...
function computeRouteAnalysis(coords, cameras) {
  ...
  const segCameraCounts = coords.slice(0, -1).map((p1, i) => {
    const p2 = coords[i + 1];
    return cameras.filter((cam) => distanceToSegmentMeters(cam.lat, cam.lon, p1[1], p1[0], p2[1], p2[0]) <= THRESHOLD).length;
  });
  ...
  const score = Math.min(100, Math.round((cameras.length / Math.max(routeMiles, 0.05)) * 8));
  const worstCount = Math.max(...segCameraCounts);
  const worstIdx = segCameraCounts.indexOf(worstCount);
  return { gradientStops, graphData, score, totalCameras: cameras.length, worstCount, worstIdx, routeMiles: routeMiles.toFixed(2), segCameraCounts, maxCount };
}
...
export default function MapView() {
  ...
  useEffect(() => {
    try {
        setTripHistory(JSON.parse(localStorage.getItem("trip_history") || "[]"));
        } catch { /* ignore */ }
    }, []);
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      map.current.addLayer({
        ...
        source: "safe-route",
        layout: { "line-cap": "round", "line-join": "round" },
        paint: { "line-color": "#2196F3", "line-width": 4, "line-dasharray": [2, 2], "line-opacity": 0.9 },
      });
      ...
      map.current.addLayer({
        ...
        paint: {
          "circle-color": ["step", ["get", "point_count"], "#FFB703", 10, "#FB8500", 50, "#E63946"],
      ...
      map.current.on("click", "camera-unclustered", (e) => {
        ...
        new mapboxgl.Popup({ offset: 10 })
          .setLngLat(coords)
          .setHTML(`<div style="font-size:12px;line-height:1.7;min-width:160px;">
  ...
  const updateRoute = (coords) => {
    ...
    map.current.getSource("route").setData({ type: "Feature", geometry: { type: "LineString", coordinates: coords } });
  };

  const resetRouteColor = () => {
    if (!map.current?.getLayer("route-line")) return;
    map.current.setPaintProperty("route-line", "line-gradient", ["interpolate", ["linear"], ["line-progress"], 0, "#E63946", 1, "#E63946"]);
  };

  const updateSafeRouteLayer = (coords) => {
    if (!map.current?.getSource("safe-route")) return;
    map.current.getSource("safe-route").setData({ type: "Feature", geometry: { type: "LineString", coordinates: coords } });
  };
  ...
  const animateRouteDraw = (coords, gradientStops, onComplete) => {
    ...
    map.current.getSource("route").setData({ type: "Feature", geometry: { type: "LineString", coordinates: [coords[0], coords[0]] } });
    map.current.setPaintProperty("route-line", "line-gradient", ["interpolate", ["linear"], ["line-progress"], 0, "#E63946", 1, "#E63946"]);
    const frame = (now) => {
      const t = Math.min((now - start) / DURATION, 1);
      const eased = 1 - Math.pow(1 - t, 3);
      try {
        const sliced = turf.lineSliceAlong(line, 0, Math.max(eased * totalLengthKm, 0.0001), { units: "kilometers" });
      ...
      if (t < 1) {
        ...
        if (gradientStops.length >= 2) {
          map.current.setPaintProperty("route-line", "line-gradient", ["interpolate", ["linear"], ["line-progress"], ...gradientStops]);
        }
        setAnimating(false);
        onComplete();
      }
  ...
  const applyAvoidanceRadius = (routes, allCams, radius) => {
    const scored = routes.map((r) => ({ ...r, cameraCount: scoreRouteByCameras(r.geometry.coordinates, allCams, radius) }));
    let safestIdx = 0;
    for (let i = 1; i < scored.length; i++) {
      if (scored[i].cameraCount < scored[safestIdx].cameraCount) safestIdx = i;
    }
    const original = scored[0], safest = scored[safestIdx];
    const primaryCameras = allCams.filter((c) => isCameraNearPath(c.lat, c.lon, original.geometry.coordinates, radius));
    setCameras(primaryCameras);
    if (safestIdx !== 0) updateSafeRouteLayer(safest.geometry.coordinates);
    else updateSafeRouteLayer([]);
    setRouteStats({
      original: { time: original.duration, distance: original.distance, cameras: original.cameraCount },
      safe: safestIdx !== 0 ? { time: safest.duration, distance: safest.distance, cameras: safest.cameraCount } : null,
    });
  };
  ...
  const createMarkerElement = () => {
    ...
    el.style.cssText = `width:16px;height:16px;background:#E63946;border:2.5px solid white;border-radius:50%;box-shadow:0 2px 6px rgba(0,0,0,0.3);cursor:pointer;`;
    return el;
  };
  ...
  const createEndpointMarker = (coord, label) => {
    ...
    const el = document.createElement("div");
    el.style.cssText = `width:28px;height:28px;background:${isStart ? "#388e3c" : "#7B1FA2"};border:2.5px solid white;border-radius:50%;box-shadow:0 2px 6px rgba(0,0,0,0.35);cursor:grab;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:white;`;
    el.innerText = label;
    const marker = new mapboxgl.Marker({ element: el, draggable: true }).setLngLat(coord).addTo(map.current);
    marker._id = id;
    marker._el = el;
    el.addEventListener("click", (e) => { e.stopPropagation(); markerClickedRef.current = true; });
    marker.on("drag", () => {
  ...
  const createMarker = (coord) => {
    ...
    const marker = new mapboxgl.Marker({ element: el, draggable: true }).setLngLat(coord).setPopup(popup).addTo(map.current);
    marker._id = id;
    marker._el = el;
    const refreshPopup = () => {
  ...
  const clearRoute = () => {
    ...
    if (style) {
      style.layers.filter(l => l.id.startsWith('route-graded-')).forEach(l => {
  ...
  const analyzeRoute = async () => {
    const canAnalyze = mode === "route" ? pointsRef.current.length === 2 : pointsRef.current.length >= 2;
    ...
    try {
      ...
      const camRes = await fetch(`/api/cameras?south=${south}&west=${west}&north=${north}&east=${east}`);
      ...
      const nearby = fetched.filter((c) => isCameraNearPath(c.lat, c.lon, primaryCoords, safeRadius));
      applyAvoidanceRadius(routes, fetched, safeRadius);
      setTimeout(() => { applyColorGradedRoute(map.current, routes[0].geometry.coordinates, fetched); }, 0);
      ...
      for (let i = 1; i < scoredTrip.length; i++) {
        if (scoredTrip[i].cameraCount < scoredTrip[tripSafeIdx].cameraCount) tripSafeIdx = i;
      }
  ...
  const saveTripToHistory = (entry) => {
    const prev = (() => { try { return JSON.parse(localStorage.getItem("trip_history") || "[]"); } catch { return []; } })();
  ...
  const scoreColor = !analysisData ? "#fff" : analysisData.score < 30 ? "#00C853" : analysisData.score < 60 ? "#FFD600" : "#E63946";
  const scoreLabel = !analysisData ? "" : analysisData.score < 30 ? "Low Risk" : analysisData.score < 60 ? "Moderate" : "High Risk";
  ...
  function interpolateRoute(coords, segmentMeters = 20) {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      for (let s = 0; s < steps; s++) {
        result.push([lon1 + (lon2 - lon1) * (s / steps), lat1 + (lat2 - lat1) * (s / steps)]);
      }
    }
  ...
  function applyColorGradedRoute(mapRef, pathCoords, cameras, thresholdMeters = 50, maxCams = 3) {
    const existingLayers = mapRef.getStyle().layers.map(l => l.id);
    existingLayers.filter(id => id.startsWith('route-graded-')).forEach(id => {
      mapRef.removeLayer(id);
      mapRef.removeSource(id);
    });
    if (pathCoords.length < 2) {
      if (mapRef.getLayer('route-line')) mapRef.setLayoutProperty('route-line', 'visibility', 'visible');
      return;
    }
    if (mapRef.getLayer('route-line')) mapRef.setLayoutProperty('route-line', 'visibility', 'none');
    const dense = interpolateRoute(pathCoords, 20);
    const segments = [];
    for (let i = 0; i < dense.length - 1; i++) {
      const count = cameras.filter(cam => isCameraNearPath(cam.lat, cam.lon, [dense[i], dense[i + 1]], thresholdMeters)).length;
      segments.push({ coords: [dense[i], dense[i + 1]], color: densityColor(Math.min(count / maxCams, 1)) });
    }
    const merged = [];
    ...
    for (let i = 0; i < segments.length; i++) {
      if (segments[i].color === current.color) {
        ...
        merged.push(current);
        current = { coords: [segments[i].coords[0], segments[i].coords[1]], color: segments[i].color };
      }
    }
    merged.push(current);
    merged.forEach((run, idx) => {
      const id = `route-graded-${idx}`;
      mapRef.addSource(id, { type: 'geojson', data: { type: 'Feature', geometry: { type: 'LineString', coordinates: run.coords } } });
      mapRef.addLayer({ id, type: 'line', source: id, layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': run.color, 'line-width': 5, 'line-opacity': 0.95 } });
    });
  }
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 8, pointerEvents: animating || showResults ? "auto" : "none", opacity: animating || panelVisible ? 1 : 0, transition: "opacity 0.4s ease" }}
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
          ...
          <div style={{ display: "flex", gap: 3, background: "#f0f0f0", borderRadius: 8, padding: 3 }}>
            {[["draw", "✏️ Free Draw"], ["route", "🗺 A→B Route"]].map(([m, label]) => (
              <button key={m} onClick={(e) => { e.stopPropagation(); switchMode(m); }}
                style={{ background: mode === m ? "white" : "transparent", border: "none", padding: "8px 16px", borderRadius: 7, fontSize: 13, fontWeight: mode === m ? 700 : 500, cursor: "pointer", color: mode === m ? "#111" : "#666", boxShadow: mode === m ? "0 1px 4px rgba(0,0,0,0.1)" : "none", transition: "all 0.15s" }}>
                {label}
              </button>
            ))}
          </div>
          <button onClick={(e) => { e.stopPropagation(); setHistoryOpen((o) => !o); }}
            style={{ background: historyOpen ? "#1976D2" : "white", border: "1px solid #e0e0e0", padding: "9px 18px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", color: historyOpen ? "white" : "#555", display: "flex", alignItems: "center", gap: 5, boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 18, letterSpacing: "-0.5px", color: "#111" }}>PANOPTICON</span>
        </div>

        {/* Right: Search + status */}
        <div style={{ display: "flex", gap: 12, alignItems: "center", flex: 1, justifyContent: "flex-end" }}>
          <div style={{ position: "relative", maxWidth: 360, flex: 1 }}>
            <input type="text" placeholder="Search a city..." value={search} onChange={(e) => handleSearch(e.target.value)}
              style={{ width: "100%", padding: "10px 16px", borderRadius: 9, border: "1px solid #ccc", fontSize: 14, outline: "none", boxSizing: "border-box", color: "#111", background: "#f5f5f5" }} />
          ...
          <span style={{ fontSize: 13, color: cameras.length > 0 ? "#c47c00" : activeEndpointId ? "#2196F3" : "#444", fontWeight: cameras.length > 0 || activeEndpointId ? 600 : 400, flexShrink: 0 }}>
            {cameras.length > 0 ? `📷 ${cameras.length} camera${cameras.length !== 1 ? "s" : ""} on route`
              : isLoading ? animating ? "Analyzing your route…" : "Fetching cameras…"
              : mode === "route" ? points.length === 0 ? "Click map to place start (A)" : points.length === 1 ? "Click map to place end (B)" : "A→B set — click Analyze Route"
              : points.length === 0 ? "Click map to add points"
              : activeEndpointId ? prependMode ? "⬆️ Adding before first point — click map" : "📍 Extending from selected point — click map"
      ...
      {showResults && analysisData && routeStats && (
        <div onClick={(e) => e.stopPropagation()} style={{ position: "absolute", top: "50%", left: "50%", transform: panelVisible ? "translate(-50%, -50%) scale(1)" : "translate(-50%, -50%) scale(0.9)", opacity: panelVisible ? 1 : 0, transition: "all 0.4s cubic-bezier(0.32, 0.72, 0, 1)", zIndex: 9, width: "90%", maxWidth: 800, textAlign: "center" }}>
          <button onClick={() => setShowResults(false)} style={{ position: "absolute", top: -8, right: 0, width: 30, height: 30, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.25)", background: "rgba(0,0,0,0.55)", backdropFilter: "blur(4px)", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", color: "white", zIndex: 10 }}>×</button>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxHeight: "80vh", overflowY: "auto", padding: "4px" }}>
            <div style={{ background: "rgba(0,0,0,0.35)", backdropFilter: "blur(12px)", borderRadius: 16, padding: "12px", display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8, color: "rgba(255,255,255,0.7)" }}>Route Preview</div>
              <MiniRouteMap coords={routeCoords} analysisData={analysisData} />
            </div>

            <div style={{ background: "rgba(0,0,0,0.35)", backdropFilter: "blur(12px)", borderRadius: 16, padding: "20px", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: "24px" }}>
              {[{ val: analysisData.score, label: "Safety Score" }, { val: analysisData.totalCameras, label: "Total Cameras" }].map(({ val, label }) => (
                <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(0,0,0,0.55)", backdropFilter: "blur(8px)", borderRadius: 20, padding: "12px 24px", flex: 1, height: "100%" }}>
                  <div style={{ fontSize: 64, fontWeight: 900, lineHeight: 1, color: scoreColor, letterSpacing: "-4px" }}>{val}</div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: scoreColor, textTransform: "uppercase", letterSpacing: 2, marginTop: 6 }}>{label}</div>
                </div>
              ))}
            </div>

            <div style={{ background: "rgba(0,0,0,0.35)", backdropFilter: "blur(12px)", borderRadius: 16, padding: "16px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 12, color: "rgba(255,255,255,0.7)" }}>Camera Exposure</div>
              <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 16 }}>
                {[{ value: `${analysisData.routeMiles}`, label: "Total Miles" }, { value: analysisData.worstCount, label: "Worst Block", color: "#E63946" }].map(({ value, label, color }) => (
                  <div key={label} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 24, fontWeight: 900, color: color || "white", letterSpacing: "-1px" }}>{value}</div>
                    <div style={{ fontSize: 9, color: "rgba(255,255,255,0.6)", textTransform: "uppercase", letterSpacing: 1, marginTop: 4 }}>{label}</div>
                  </div>
                ))}
              </div>
              <ResponsiveContainer width="100%" height={60}>
                <AreaChart data={analysisData.graphData} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="distanceMi" tick={{ fontSize: 8, fill: "rgba(255,255,255,0.5)" }} tickLine={false} axisLine={false} />
                  <YAxis tick={{ fontSize: 8, fill: "rgba(255,255,255,0.5)" }} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(0,0,0,0.8)", color: "#fff" }} formatter={(v) => [`${v} cameras`, "Exposure"]} labelFormatter={(l) => `${l} mi`} />
                  <Area type="monotone" dataKey="cameras" stroke="#E63946" strokeWidth={2} fill="url(#exposureGradient)" isAnimationActive={true} animationDuration={1000} />
                </AreaChart>
              </ResponsiveContainer>
              <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                {[["#00C853", "Low"], ["#FFD600", "Medium"], ["#FF6D00", "High"], ["#E63946", "Severe"]].map(([color, label]) => (
                  <div key={label} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "rgba(255,255,255,0.6)" }}>
            ...
            <div style={{ background: "rgba(0,0,0,0.35)", backdropFilter: "blur(12px)", borderRadius: 16, padding: "16px", display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 12, color: "rgba(255,255,255,0.7)" }}>Route Comparison</div>
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 10, background: "rgba(230, 57, 70, 0.2)", border: "1px solid rgba(230, 57, 70, 0.3)" }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 4, textTransform: "uppercase" }}>Your Route</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#E63946" }}>{formatTime(routeStats.original.time)}</div>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginTop: 2 }}>{formatDistance(routeStats.original.distance)}</div>
                  <div style={{ fontSize: 11, color: "#FFB703", marginTop: 4, fontWeight: 600 }}>📷 {routeStats.original.cameras} camera{routeStats.original.cameras !== 1 ? "s" : ""}</div>
                </div>
                <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 10, background: routeStats.safe ? "rgba(33, 150, 243, 0.2)" : "rgba(255,255,255,0.05)", border: `1px solid ${routeStats.safe ? "rgba(33, 150, 243, 0.3)" : "rgba(255,255,255,0.1)"}` }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 4, textTransform: "uppercase" }}>Safe Route</div>
                  {routeStats.safe ? (
                    <>
                      <div style={{ fontSize: 20, fontWeight: 800, color: "#4CAF50" }}>{formatTime(routeStats.safe.time)}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginTop: 2 }}>{formatDistance(routeStats.safe.distance)}</div>
                      <div style={{ fontSize: 11, color: "#4CAF50", marginTop: 4, fontWeight: 600 }}>📷 {routeStats.safe.cameras} camera{routeStats.safe.cameras !== 1 ? "s" : ""}</div>
                    </>
                  ) : (
                    <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 12 }}>No safer alternative found</div>
                  )}
                </div>
              </div>
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>
                  ...
                  <span style={{ fontWeight: 600, color: "#2196F3" }}>{safeRadius} m ({Math.round(safeRadius * 3.281)} ft)</span>
                </div>
                <input type="range" min={10} max={200} step={5} value={safeRadius} onChange={(e) => setSafeRadius(parseInt(e.target.value))} style={{ width: "100%", accentColor: "#2196F3" }} />
      ...
      {/* Bottom action bar */}
      <div onClick={(e) => e.stopPropagation()} style={{ position: "absolute", bottom: 30, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 10, zIndex: 10, alignItems: "center" }}>
        <div style={{ display: "flex", gap: 3, background: "white", borderRadius: 10, padding: 3, border: "1px solid #e0e0e0", boxShadow: "0 2px 12px rgba(0,0,0,0.12)" }}>
          {[["walking", "🚶 Walk"], ["driving", "🚗 Drive"]].map(([p, label]) => (
            <button key={p} onClick={(e) => { e.stopPropagation(); setProfile(p); }}
              style={{ background: profile === p ? "#2196F3" : "transparent", border: "none", padding: "11px 18px", borderRadius: 8, fontSize: 13, fontWeight: profile === p ? 700 : 500, cursor: "pointer", color: profile === p ? "white" : "#666", transition: "all 0.15s" }}>
              {label}
            </button>
          ))}
        </div>
        <button onClick={(e) => { e.stopPropagation(); clearRoute(); }} style={{ background: "white", border: "1px solid #e0e0e0", padding: "14px 24px", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: "0 2px 12px rgba(0,0,0,0.12)", display: "flex", alignItems: "center", gap: 6, color: "#111" }}>
          🗑️ Clear
        </button>
        <button onClick={(e) => { e.stopPropagation(); analyzeRoute(); }} disabled={!canAnalyze || isLoading}
          style={{ background: !canAnalyze || isLoading ? "#ccc" : "#E63946", color: "white", border: "none", padding: "14px 28px", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: !canAnalyze || isLoading ? "not-allowed" : "pointer", boxShadow: canAnalyze && !isLoading ? "0 2px 12px rgba(230,57,70,0.4)" : "none", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {/* History panel */}
      {historyOpen && (
        <div onClick={(e) => e.stopPropagation()} style={{ position: "absolute", top: 61, right: 0, bottom: 0, width: 320, background: "white", boxShadow: "-4px 0 24px rgba(0,0,0,0.12)", zIndex: 15, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
            <span style={{ fontWeight: 700, fontSize: 14, color: "#111" }}>Trip History</span>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {tripHistory.length > 0 && (
                <button onClick={() => { localStorage.removeItem("trip_history"); setTripHistory([]); }} style={{ background: "none", border: "none", fontSize: 11, color: "#bbb", cursor: "pointer", padding: "2px 4px" }}>Clear all</button>
              )}
              <button onClick={() => setHistoryOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#999", fontSize: 18, padding: "0 2px", lineHeight: 1 }}>✕</button>
            </div>
          </div>
          <div style={{ overflowY: "auto", flex: 1, padding: "8px 0" }}>
            {tripHistory.length === 0 ? (
              <div style={{ color: "#bbb", fontSize: 13, textAlign: "center", marginTop: 48, lineHeight: 1.7 }}>No trips yet.<br />Analyze a route to save it here.</div>
            ) : (
              tripHistory.map((trip) => (
                <div key={trip.id} style={{ margin: "0 12px 8px", padding: "12px", borderRadius: 10, background: "#f9f9f9", border: "1px solid #eee" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <span style={{ fontSize: 11, color: "#999" }}>{formatRelativeTime(trip.timestamp)}</span>
                    <div style={{ display: "flex", gap: 4 }}>
                      <span style={{ fontSize: 10, background: "#e8f5e9", color: "#388e3c", padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>{trip.profile === "driving" ? "🚗" : "🚶"} {trip.profile}</span>
                      <span style={{ fontSize: 10, background: "#e3f2fd", color: "#1565c0", padding: "2px 6px", borderRadius: 4, fontWeight: 600 }}>{trip.mode === "route" ? "A→B" : "Free Draw"}</span>
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    <div style={{ background: "#fff5f5", borderRadius: 7, padding: "8px", textAlign: "center", border: "1px solid #fcc" }}>
                      ...
                      <div style={{ fontWeight: 700, fontSize: 15, color: "#E63946" }}>{formatTime(trip.original.duration)}</div>
                      <div style={{ fontSize: 11, color: "#888" }}>{formatDistance(trip.original.distance)}</div>
                      <div style={{ fontSize: 11, color: "#c47c00", fontWeight: 600, marginTop: 2 }}>📷 {trip.original.cameraCount}</div>
                    </div>
                    <div style={{ background: trip.safe ? "#f0f7ff" : "#f9f9f9", borderRadius: 7, padding: "8px", textAlign: "center", border: `1px solid ${trip.safe ? "#90caf9" : "#eee"}` }}>
                      <div style={{ fontSize: 10, color: "#999", marginBottom: 2, textTransform: "uppercase" }}>Safe Route</div>
                      {trip.safe ? (
                        <>
                          <div style={{ fontWeight: 700, fontSize: 15, color: "#2196F3" }}>{formatTime(trip.safe.duration)}</div>
                          <div style={{ fontSize: 11, color: "#888" }}>{formatDistance(trip.safe.distance)}</div>
                          <div style={{ fontSize: 11, color: "#388e3c", fontWeight: 600, marginTop: 2 }}>📷 {trip.safe.cameraCount}</div>
                        </>
                      ) : (
                  ...
                  <button onClick={() => { if (trip.waypoints?.length > 0) { const [lon, lat] = trip.waypoints[0]; map.current?.flyTo({ center: [lon, lat], zoom: 14, duration: 1200 }); } }}
                    style={{ marginTop: 8, width: "100%", background: "none", border: "1px solid #e0e0e0", borderRadius: 6, padding: "5px", fontSize: 11, color: "#666", cursor: "pointer" }}>
                    📍 Fly to start
                  </button>
                </div>
              ))
